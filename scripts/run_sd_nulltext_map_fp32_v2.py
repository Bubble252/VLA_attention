...
def main() -> int:
    ...
    pipe = StableDiffusionPipeline.from_pretrained(args.model, torch_dtype=torch.float32, local_files_only=True, safety_checker=None, requires_safety_checker=False).to("cuda")
    ...
    pixels = torch.from_numpy(np.asarray(image).copy()).permute(2, 0, 1).unsqueeze(0).float().div(127.5).sub(1).to("cuda", dtype=torch.float32)
    with torch.no_grad():
