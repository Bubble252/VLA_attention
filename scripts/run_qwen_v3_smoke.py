...
def main() -> int:
    ...
    p=argparse.ArgumentParser(); p.add_argument('--model',type=Path,required=True); p.add_argument('--dataset-root',type=Path,required=True); p.add_argument('--manifest',type=Path,required=True); p.add_argument('--cache',type=Path,required=True); p.add_argument('--lora-config',type=Path,required=True); p.add_argument('--output',type=Path,required=True); p.add_argument('--checkpoint',type=Path,default=None); p.add_argument('--max-steps',type=int,default=20); p.add_argument('--lambda-sem',type=float,default=0.1); p.add_argument('--teacher-temperature',type=float,default=1.0); p.add_argument('--semantic-warmup-steps',type=int,default=0); p.add_argument('--semantic-loss-mode',choices=['kl','js','kl_rank','kl_moment'],default='kl'); p.add_argument('--common-resolution',type=int,default=32); p.add_argument('--rank-quantile',type=float,default=.25); p.add_argument('--rank-margin',type=float,default=.5); p.add_argument('--rank-weight',type=float,default=1.0); p.add_argument('--moment-weight',type=float,default=1.0); p.add_argument('--seed',type=int,default=17); a=p.parse_args()
    ...
    try:
      for step in range(a.max_steps):
        ...
        common=a.common_resolution; student=resample_spatial_distribution(student,source_height=gh//merge,source_width=gw//merge,target_height=common,target_width=common); teacher=resample_spatial_distribution(teacher,source_height=gh//merge,source_width=gw//merge,target_height=common,target_width=common); sem=semantic_map_loss(student,teacher,mode=a.semantic_loss_mode,teacher_temperature=a.teacher_temperature,rank_quantile=a.rank_quantile,rank_margin=a.rank_margin,rank_weight=a.rank_weight,moment_weight=a.moment_weight); warmup=min(1.0,(step+1)/a.semantic_warmup_steps) if a.semantic_warmup_steps else 1.0; effective_lambda=a.lambda_sem*warmup; loss=out.loss+effective_lambda*sem
        if not torch.isfinite(loss): raise RuntimeError('non-finite V3 loss')
        opt.zero_grad(set_to_none=True); loss.backward(); grad_pre=float(torch.nn.utils.clip_grad_norm_(model.parameters(),float('inf'))); grad_post=float(torch.nn.utils.clip_grad_norm_(model.parameters(),cfg['optimizer']['gradient_clip_norm'])); opt.step(); logs.append({'total':float(loss.detach()),'caption':float(out.loss.detach()),'semantic':float(sem.detach()),'effective_lambda_sem':effective_lambda,'grad_norm_pre_clip':grad_pre,'grad_norm_post_clip':min(grad_pre,cfg['optimizer']['gradient_clip_norm'])})
    finally: h.remove()
