...
def main():
 ...
 p=argparse.ArgumentParser(); p.add_argument('--model',type=Path,required=True); p.add_argument('--dino',type=Path,required=True); p.add_argument('--dataset-root',type=Path,required=True); p.add_argument('--manifest',type=Path,required=True); p.add_argument('--cache',type=Path,required=True); p.add_argument('--lora-config',type=Path,required=True); p.add_argument('--output',type=Path,required=True); p.add_argument('--max-steps',type=int,default=20); p.add_argument('--lambda-ret',type=float,default=.1); p.add_argument('--lambda-sem',type=float,default=.1); p.add_argument('--seed',type=int,default=17); a=p.parse_args()
 ...
 try:
  for step in range(a.max_steps):
   ...
   ret=cosine_retention_loss(feat,projector(teacher_feat)); sem=semantic_map_kl(student,teacher); loss=out.loss+a.lambda_ret*ret+a.lambda_sem*sem; opt.zero_grad(set_to_none=True); loss.backward(); torch.nn.utils.clip_grad_norm_(list(model.parameters())+list(projector.parameters()),cfg['optimizer']['gradient_clip_norm']); opt.step(); logs.append({'total':float(loss.detach()),'caption':float(out.loss.detach()),'retention':float(ret.detach()),'semantic':float(sem.detach())})
 finally: h.remove()
 vis=[n for n,x in model.named_parameters() if x.requires_grad and '.visual.' in n and 'lora_' in n]; a.output.parent.mkdir(parents=True,exist_ok=True); a.output.write_text(json.dumps({'experiment':'V4-caption-plus-retention-plus-semantic-smoke','steps':a.max_steps,'lambda_ret':a.lambda_ret,'lambda_sem':a.lambda_sem,'losses':logs,'visual_lora_tensors':len(vis),'dino_frozen':True},indent=2)+'\n'); print(a.output)
